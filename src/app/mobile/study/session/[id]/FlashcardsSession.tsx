...
export function FlashcardsSession({ sessionId, language, completed = false }: { sessionId: string; language: 'en' | 'ko'; completed?: boolean }) {
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-10 text-center gap-4">
        ...
        <div className="w-full max-w-xs flex flex-col gap-2 mt-2">
          <Link href="/mobile/study/subscription" className="w-full lg:w-auto lg:min-w-[200px] inline-flex items-center justify-center h-11 rounded-full bg-primary text-white text-[15px] font-semibold">
            {ko ? '프리미엄 보기' : 'See Premium'}
          </Link>
          <Link href="/mobile/study" className="w-full lg:w-auto lg:min-w-[200px] inline-flex items-center justify-center h-11 rounded-full bg-white border border-gray-200 text-[15px] font-medium text-gray-700">
  ...
  if (gate === 'limit') {
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-10 text-center gap-4">
        ...
        <Link href="/mobile/study" className="w-full max-w-xs inline-flex items-center justify-center h-11 rounded-full bg-primary text-white text-[15px] font-semibold">
          {t('study.flashcards.backToStudy')}
        </Link>
      </div>
    )
        ...
        <ul className="flex flex-col gap-2">
          {deck.map((c, i) => (
            <li key={i} className="rounded-2xl bg-white ring-1 ring-gray-200/70 p-4">
              <p className="text-[15px] font-semibold text-gray-900 leading-snug whitespace-pre-wrap">{c.front}</p>
  ...
  if (error || !deck) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-5 text-center gap-3">
        ...
        <p className="text-[15px] text-gray-600">{t('study.flashcards.loadFailed')}</p>
        <button
          type="button"
          onClick={() => void load()}
  ...
  if (deck.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-5 py-8 text-center gap-3">
        ...
        <Link
          href="/mobile/study"
          className="inline-flex items-center justify-center h-10 px-5 rounded-full bg-primary text-white text-[15px] font-medium"
        >
      ...
      <div className="flex-1 overflow-y-auto px-5 py-8 flex flex-col items-center">
        ...
        <div className="w-full max-w-sm rounded-3xl bg-gradient-to-br from-violet-500 via-purple-600 to-indigo-700 text-white p-6 text-center shadow-[0_18px_40px_-16px_rgba(124,58,237,0.55)] relative overflow-hidden">
          ...
          <div className="relative mt-1 flex items-end justify-center gap-1.5">
            <span className="text-[48px] font-black tracking-tight tabular-nums leading-none">{marked.got}</span>
            <span className="text-[24px] font-bold text-white/70 leading-none mb-0.5">/ {deck.length}</span>
          </div>
          ...
          <div className="relative mt-4 flex items-center justify-center gap-2">
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-white/15 text-[13px] font-semibold">
        ...
        <div className="mt-4 w-full max-w-sm">
          <button
            ...
            className="w-full flex items-center justify-between gap-2 px-4 h-11 rounded-2xl bg-white ring-1 ring-gray-200/70 text-[13px] font-semibold text-gray-800 hover:bg-gray-50 transition"
          >
        ...
        <div className="mt-4 w-full max-w-sm flex flex-col gap-2">
          <button
            type="button"
            onClick={() => void startFreshDeck()}
            className="w-full lg:w-auto lg:min-w-[200px] inline-flex items-center justify-center gap-1.5 h-11 rounded-full bg-primary text-white text-[15px] font-semibold active:scale-[0.98] transition"
          >
            <RefreshCw className="w-4 h-4" />
            {t('study.flashcards.newDeck')}
          </button>
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      {errorToast}
      <div className="flex-shrink-0 px-5 py-3 flex items-center justify-between text-[13px] text-gray-500">
        <span>
        ...
        >
          ...
          >
            ...
            >
              ...
              {card.hint && (
                showHint ? (
                  <div className="mt-2 px-4 py-2 rounded-xl bg-amber-50 border border-amber-100 text-[13px] text-amber-800 inline-flex items-start gap-1.5 text-left">
                  ...
                  <span
                    ...
                    className="mt-1 text-[13px] text-amber-700 inline-flex items-center gap-1 cursor-pointer"
                  >
            ...
            >
              ...
              <p className="text-[17px] font-medium leading-relaxed whitespace-pre-wrap">
                {card.back}
              </p>
            </div>
          </div>
