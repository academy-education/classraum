    ...
    <Card className={`${className} p-4`}>
      <div className="flex items-start gap-3 mb-3">
        <div className="w-9 h-9 rounded-xl bg-indigo-50 flex items-center justify-center flex-shrink-0">
          <Tent className="w-4 h-4 text-indigo-600" strokeWidth={1.75} />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-gray-500">
            {t('mobile.campWork.title')}
          </p>
          <p className="text-[15px] font-semibold tracking-tight text-gray-900 truncate leading-snug">
            {heading}
          </p>
        </div>
        <span className={`flex-shrink-0 text-[11px] font-semibold px-2 py-0.5 rounded-md tabular-nums ${
        ...
        {preview.map(a => {
          ...
          return (
            <button
              ...
              className="flex items-center gap-3 w-full text-left rounded-xl ring-1 ring-gray-100 bg-gray-50/60 px-3 py-2.5 min-h-[56px] active:scale-[0.995] transition-all disabled:opacity-60"
            >
              <div className="flex-1 min-w-0">
                <div className="text-[14px] font-semibold text-gray-900 truncate leading-snug">
                  {a.title}
                </div>
                {from && <div className="text-[11px] text-gray-500 truncate mt-0.5">{from}</div>}
                <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                  <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-white ring-1 ring-gray-100 text-[11px] font-medium text-gray-600 tabular-nums">
                  ...
                  {done ? (
                    ...
                    <span className="inline-flex items-center px-1.5 py-0.5 rounded-md bg-blue-50 text-[11px] font-semibold text-blue-700">
                      {String(t('study.camp.inProgress'))}
                    </span>
                  ) : (
                    <span className="inline-flex items-center px-1.5 py-0.5 rounded-md bg-white ring-1 ring-gray-100 text-[11px] font-medium text-gray-600">
      ...
      {rows.length > PREVIEW_COUNT && (
        <button
          ...
          className="mt-3 w-full min-h-[44px] rounded-xl text-[13px] font-semibold text-primary hover:bg-primary/[0.06] transition-colors"
        >
