...
export const WORK: WorkItem[] = [
  ...
  },
  {
    ...
    size: '2 of 4 fixed — insert paths built, gate wired; 2 decisions left',
    ...
    owner: 'claude',
    state: 'open',
    note: 'TWO FIXED 2026-08-09. (3) fill_in_blanks, speaking_interview and arrange_words now have insert commands — `insert-fill-in-blanks`, `insert-arrange-words`, `insert-interview` — so the 249 live items of these types can finally be grown rather than only served. Each carries a shape rule derived from the live rows and validated BOTH ways: it accepts 93/93, 108/108 and 48/48 of what is already banked, and `frozen-item-shapes.test.ts` pins what it rejects. The blanks rule is why the live check came first — the first draft counted underscore runs and matched 0 of 93, because the marker is "[n]"; fixtures written alongside the rule would have agreed with it. The arrange_words rule is the strongest: correct_answer must be a permutation of choices, since a student can only ever emit a permutation and any other answer is unreachable. (4) The ledger gate now runs on EVERY insert path — insertRepeat and insertWriting reached the bank without it, so the gate documented as the pipeline floor was enforced on one path of three. STILL OPEN, both decisions rather than bugs: (1) shuffleInPlace is dead in both SAT helpers while its own comment claims otherwise — measured, no damage done (key spread 30/27/23/20), but wiring it changes what gets banked; (2) accepts() carries two carve-outs the runbook never mentions (Standard English Conventions bypasses to a unanimous 3/3, Rhetorical Synthesis returns ok before the distractor and passage checks) — either document them or remove them.',
  },
  {
    id: 'A13',
