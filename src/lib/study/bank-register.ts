...
export const WORK: WorkItem[] = [
  ...
  },
  {
    ...
    state: 'open',
    account: 'Whoever sits it, on THEIR OWN login. This field said support@classraum.com because it assumed Andy would sit it; if the co-founder does, they must use andy.manager@gmail.com. Reviewer identity IS the account (study_item_reviews keys on the logged-in user), so two people sharing one login merge into one reviewer and no agreement number from it survives. Unlike B1 there is no collision risk from either choice — neither cohort has any human review yet — but the sitting must be attributable to the person who actually sat it.',
