...
for(let from=0;;from+=1000){
  const {data,error}=await db.from('study_item_bank').select('id,subskill,cohort,difficulty,verified,archived,item').eq('family','sat').eq('domain','Craft and Structure').range(from,from+999)
...
const b=JSON.parse(readFileSync('scripts/study-bank/live-cs-control.batch.json','utf8'))
let hit=0
for(const it of b){
