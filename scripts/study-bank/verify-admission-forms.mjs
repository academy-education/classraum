...
for (const [family, sections] of Object.entries(BLUEPRINT)) {
  ...
  for (const [section, want] of Object.entries(need)) {
    ...
    console.log(`  ${section.padEnd(8)} need ${String(want).padStart(3)}  drawable ${String(d.drawable).padStart(3)}  ${ok ? 'OK' : 'SHORT'}  margin ${margin >= 0 ? '+' : ''}${margin}${capNote}`)
    if (!ok) failed = true
