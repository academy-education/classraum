...
for(let from=0;;from+=1000){
  const {data,error}=await db.from('study_item_bank').select('id,family,domain,subskill,cohort,verified,archived,difficulty,passage_group_id,content_sha,created_at,item').eq('family','sat').in('domain',[process.env.DOM]).range(from,from+999)
