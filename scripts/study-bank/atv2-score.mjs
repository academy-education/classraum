...
const key = JSON.parse(readFileSync(`${DIR}/atv2-pilot.key.json`, 'utf8'))
const ns = Object.keys(key)
if (ns.length !== 24) { console.error(`FAIL: key has ${ns.length} entries, expected 24`); process.exit(1) }
...
for (const s of solvers) {
  const picks = JSON.parse(readFileSync(`${DIR}/atv2-pilot.solver-${s}.json`, 'utf8'))
...
for (const r of results) {
  console.log(`solver ${r.s}   ${String(r.correct).padStart(2)}/24 = ${r.pct.toFixed(1)}%   confident ${r.confCorrect}/${r.confident}   spread ${LETTERS.map(l => r.spread[l]).join('/')}`)
}
...
console.log(`all-3-solved ${allSolved}/24   zero-solver items ${noneSolved}/24`)
...
if (!shift) {
  ...
  console.log(`pre-registered verdict at n=24: ${verdict}`)
}
