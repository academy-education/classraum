 ...
 * see it: the chip pool lowercases every chunk (src/lib/study/chip-display.ts,
 * pinned by chip-display.test.ts). Rewriting 44 live rows would be churn
 * with no student-visible effect, and every touched item is a fresh chance
 * to introduce a defect.
 *
 * This exists so the number stays honest as the cohort grows, and so
 * nobody re-discovers the tell and "fixes" the data instead of the render.
 * If it ever reports a leak on a surface that does NOT lowercase, that
 * surface is the bug.
 */
import fs from 'fs'
