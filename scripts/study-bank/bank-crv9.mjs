...
const ITEMS_FILE = join(HERE, 'crv9-items.json')
const COHORT = 'cr-v9'
...
if (items.length !== 36) { console.error(`expected 36 items, file has ${items.length}`); process.exit(1) }
...
async function insert() {
  ...
  for (const [i, raw] of items.entries()) {
    ...
    const { error } = await db.from('study_item_bank').insert({
      ...
      verify_meta: {
        source: 'crv9', shipped: '2026-08-28', passage_needed: true,
