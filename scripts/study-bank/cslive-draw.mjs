/**
 * Stratified live-C&S blind draw. Keys DEALT FLAT (6/6/6/6 per 24-item file)
...
for(let from=0;;from+=1000){
  const {data,error}=await db.from('study_item_bank').select('id,subskill,cohort,difficulty,item').eq('family','sat').eq('domain','Craft and Structure').eq('verified',true).eq('archived',false).range(from,from+999)
  if(error)throw error; rows.push(...data); if(data.length<1000) break
}
if(rows.length!==228) console.warn('WARNING population changed:',rows.length)

// exclude the 24 already attacked this morning (prompt+passage key)
const prior=new Set(JSON.parse(readFileSync('scripts/study-bank/live-cs-control.batch.json','utf8')).map(it=>norm(it.prompt)+'||'+norm(it.passage)))
...
const PLAN = {
  'v2::words in context': 8,
  'v2::command of evidence': 11,
  'v2::inferences': 11,
  'v2::cross-text connections': 12,
  'v2::text structure and purpose': 12,
  'rw-v7-cs-hard::words in context': 6,
  'rw-v7-cs-hard::text structure and purpose': 6,
  'rw-v7-cs-hard::cross-text connections': 6,
}
const K=3
...
for(const [k,n] of Object.entries(PLAN)){
  ...
  // round-robin so every file is itself stratified; cursor carries across strata
...
files.forEach((list,fi)=>{
  ...
  // DEAL KEYS FLAT: 6 of each letter, assigned to a shuffled item order.
  ...
  writeFileSync(`scripts/study-bank/cslive-f${fi+1}.blind.json`, JSON.stringify(blind,null,1))
  writeFileSync(`scripts/study-bank/cslive-f${fi+1}.key.json`, JSON.stringify(keyf,null,1))
  writeFileSync(`scripts/study-bank/cslive-f${fi+1}.meta.json`, JSON.stringify(meta,null,1))
...
// no duplicate items across files
