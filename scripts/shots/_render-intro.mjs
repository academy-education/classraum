// Build + render the Classraum introduction.
//
//   node scripts/shots/_render-intro.mjs <scratchpad>
//
// Reads  classraum-introduction.src.html  (authored, with __IMG_name__ tokens)
...
let html = readFileSync(SP + '/classraum-introduction.src.html', 'utf8')
html = html.replace(/__IMG_([\w-]+)__/g, (_, name) => {
  const f = `${SP}/img2-en/${name}.b64`
...
else writeFileSync(SP + '/classraum-introduction.html', html)
...
writeFileSync(SP + '/_intro-wrapped.html', wrapped)

const b = await puppeteer.launch({ headless: 'new' })
const p = await b.newPage()
await p.goto('file://' + SP + '/_intro-wrapped.html', { waitUntil: 'load', timeout: 120000 })
...
await p.pdf({ path: SP + '/classraum-introduction' + (B5 ? '-b5' : '') + (PRINT16 ? '-print16' : '') + (BLEED ? '-bleed' : '') + '.pdf', format: 'A4', printBackground: true, preferCSSPageSize: true })
...
for (let i = 0; i < pages.length; i++) await pages[i].screenshot({ path: `${SP}/_intro-p${i + 1}.png` })
await b.close()
